...
class UserMiddleware {
    async getByIdOrThrow(req, res, next) {
        try {
            ...
            if (!user) {
                throw new apiError_1.ApiError("user not found", 422);
            }
    ...
    async createUserValidator(req, res, next) {
        try {
            const { error, value } = validators_1.UserValidator.userCreate.validate(req.body);
            if (error) {
    ...
    async isUserValidUpdate(req, res, next) {
        try {
            const { error, value } = validators_1.UserValidator.userUpdate.validate(req.body);
            if (error) {
                throw new apiError_1.ApiError(error.message, 400);
            }
            req.body = value;
